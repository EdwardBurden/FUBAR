...
namespace FUBAR
{
    ...
    public static class Formations
    {
        ...
        public static List<Vector3> OrganiseRelaxedFormation<T>(Vector3 start, List<T> agents, int columns, int space)
        {
            ...
            {
                Vector3 pos = CalcRelaxedPosition(i, columns, space);
        ...
        static Vector2 CalcRelaxedPosition(int index, int columns, int space) // call this func for all your objects
        {
            float posX = (index % columns) * (space*Random.Range(0.0f,1.0f));
            float posY = (index / columns) * (space * Random.Range(0.0f, 1.0f));
